...
const defaultUsers: AppUser[] = [
  { username: "renjesus", password: "admin123", name: "Ren Jesus", role: "ADM" },
];
...
function LoginScreen({ users, onLogin, onCreateUser }: { users: AppUser[]; onLogin: (user: AppUser) => void; onCreateUser: (user: AppUser) => void }) {
  ...
  return <main className="flex min-h-screen items-center justify-center bg-[#eef4f7] p-4"><div className="w-full max-w-md overflow-hidden rounded-2xl bg-white shadow-xl shadow-slate-300/30"><section className="hidden"><div><div className="flex items-center gap-3"><div className="flex size-11 items-center justify-center rounded-2xl bg-cyan-400 text-[#102a43]"><LayoutDashboard className="size-6" /></div><span className="text-lg font-bold">Integração Inicial</span></div><div className="mt-24 max-w-md"><p className="text-sm font-bold uppercase tracking-[0.2em] text-cyan-300">Gestão inteligente</p><h1 className="mt-4 text-5xl font-bold leading-tight">Pessoas certas, integradas desde o primeiro dia.</h1><p className="mt-6 text-base leading-7 text-slate-300">Centralize entrevistas, turmas, capacitação e acompanhamento dos operadores em um único ambiente.</p></div></div><p className="text-sm text-slate-400">Acesso seguro para equipes de RH</p></section><section className="p-7 sm:p-10"><div className="mx-auto max-w-sm"><div className="flex size-11 items-center justify-center rounded-xl bg-cyan-600 text-white"><LayoutDashboard className="size-6" /></div><h1 className="mt-6 text-2xl font-bold text-[#102a43]">Integração Inicial</h1><h2 className="mt-8 text-3xl font-bold text-[#102a43]">{mode === "login" ? "Login" : "Criar usuário"}</h2><form onSubmit={submit} className="mt-8 space-y-4">{mode === "create" && <label className="block text-sm font-semibold text-slate-700">Nome completo<input value={name} onChange={(event) => setName(event.target.value)} className="mt-2 h-11 w-full rounded-xl border border-slate-200 px-3 outline-none focus:border-cyan-500 focus:ring-4 focus:ring-cyan-500/10" placeholder="Ex.: Maria Souza" /></label>}<label className="block text-sm font-semibold text-slate-700">Usuário<input value={username} onChange={(event) => setUsername(event.target.value)} autoComplete="username" className="mt-2 h-11 w-full rounded-xl border border-slate-200 px-3 outline-none focus:border-cyan-500 focus:ring-4 focus:ring-cyan-500/10" placeholder="Digite seu usuário" /></label><label className="block text-sm font-semibold text-slate-700">Senha<input value={password} onChange={(event) => setPassword(event.target.value)} type="password" autoComplete={mode === "login" ? "current-password" : "new-password"} className="mt-2 h-11 w-full rounded-xl border border-slate-200 px-3 outline-none focus:border-cyan-500 focus:ring-4 focus:ring-cyan-500/10" placeholder="Digite sua senha" /></label>{error && <p className={`rounded-lg px-3 py-2 text-sm ${error.includes("criado") ? "bg-emerald-50 text-emerald-700" : "bg-rose-50 text-rose-700"}`}>{error}</p>}<button type="submit" className="h-11 w-full rounded-xl bg-[#102a43] text-sm font-bold text-white transition hover:bg-[#1d4a6c]">{mode === "login" ? "Entrar no painel" : "Criar usuário"}</button></form><button type="button" onClick={() => { setMode(mode === "login" ? "create" : "login"); setError(""); }} className="mt-5 w-full text-sm font-semibold text-cyan-700 hover:text-cyan-900">{mode === "login" ? "Criar novo usuário" : "Voltar para o login"}</button></div></section></div></main>;
}

export default function Page() {
  const [users, setUsers] = useState<AppUser[]>(() => readLocal<AppUser[]>("integracao-users", defaultUsers).map((user) => user.username.toLowerCase() === "renjesus" ? { ...user, password: "admin123", role: "ADM" } : user));
  ...
  <div className="grid gap-4 sm:grid-cols-3">
      ...
      {view === "training" && <section className="rounded-2xl border border-cyan-100 bg-gradient-to-r from-cyan-50 to-white p-5 shadow-sm">
        ...
        <div className="mt-5 grid gap-4 sm:grid-cols-3">{[
          ...
          { label: "Treinados aplicados", value: selectedTrainees.filter((trainee) => trainee.trainingStatus === "Aplicado").length, detail: "treinamento aplicado", icon: Check, tone: "green" },
          { label: "Treinados pendentes", value: selectedTrainees.filter((trainee) => trainee.trainingStatus !== "Aplicado").length, detail: "aguardando aplicação", icon: Clock3, tone: "amber" },
        ].map(({ label, value, detail, icon: Icon, tone }) => <div key={label} className="rounded-xl border border-slate-200 bg-white p-5 shadow-sm"><div className={`flex h-10 w-10 items-center justify-center rounded-lg ${tone === "green" ? "bg-emerald-50 text-emerald-600" : tone === "amber" ? "bg-amber-50 text-amber-600" : "bg-blue-50 text-blue-600"}`}><Icon className="h-5 w-5" /></div><p className="mt-4 text-sm text-slate-500">{label}</p><div className="mt-1 flex items-end gap-2"><p className="text-2xl font-bold text-[#102a43]">{String(value).padStart(2, "0")}</p><span className="mb-1 text-[11px] font-semibold text-slate-400">{detail}</span></div></div>)}
        </div>
      </section>}
